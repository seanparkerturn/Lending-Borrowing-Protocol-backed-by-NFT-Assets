...
contract NFTVaultFactory is Auth {
    ...
    event VaultDeployed(Vault vault, ERC721 underlying);
    ...
    function deployVault(ERC721 underlying) external returns (Vault vault) {
    ...
    function getVaultFromUnderlying(ERC721 underlying) external view returns (Vault) {
        return
            Vault(
                payable(
                    keccak256(
